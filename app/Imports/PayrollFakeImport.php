<?php
// app/Imports/PayrollFakeImport.php
...
class PayrollFakeImport implements WithMultipleSheets
{
    use Importable;
    ...
    {
        $this->payrollSheet = new PayrollFakeDataSheet();
...
class PayrollFakeDataSheet implements ToCollection, WithHeadingRow, SkipsOnError, SkipsOnFailure
{
    ...
    {
        Log::info('Starting Payroll Fake Import', ['total_rows' => $rows->count()]);
        ...
        Log::info('Payroll Fake Import Completed', [
    ...
    private function preloadKaryawan(array $absenKaryawanIds)
    {
        ...
        try {
            ...
            Log::error('Error preloading karyawan', ['error' => $e->getMessage()]);
        }
    }
    ...
    private function preloadCompanies(array $absenCompanyIds)
    {
        ...
        try {
            ...
            Log::error('Error preloading companies', ['error' => $e->getMessage()]);
        }
    }
    ...
    private function validateRow($row, $rowNumber)
    {
        ...
        if (empty($absenKaryawanId) || !is_numeric($absenKaryawanId)) {
            ...
            if (!isset($this->karyawanMap[$absenKaryawanId])) {
                $errors[] = "karyawan_id '{$absenKaryawanId}' tidak ditemukan di database";
            } else {
        ...
        if (!empty($absenCompanyId)) {
            if (!is_numeric($absenCompanyId)) {
                ...
                if (!isset($this->companyMap[$absenCompanyId])) {
                    $errors[] = "company_id '{$absenCompanyId}' tidak ditemukan di database";
                } else {
        ...
        if (empty($errors) && !empty($data['periode']) && !empty($data['karyawan_id'])) {
            $exists = PayrollsFake::where('periode', $data['periode'])
                                ->where('karyawan_id', $data['karyawan_id'])
                                ->exists();
            if ($exists) {
                $errors[] = "Payroll fake untuk periode {$data['periode']} dan karyawan ini sudah ada";
            }
        }
